...
public class Dificil extends JFrame {
	...
	ImageIcon carro[] = new ImageIcon[3];
	int pontos = 0, alpha = 255, y = 60, tipo[] = new int[3];
	String letras[] = { "PARDAL", "ESTADO", "BRASIL", "JOGOS", "QUATRO", "LETRA",
			"SONO", "GATO", "PATO", "CARRO", "MURO", "JOGO", "PINO", "TREM",
			"SAPO", "MEIA", "AMOR", "SALA", "LAMA", "BALA", "ANEL", "DOCE",
			"BOLO", "NOVE", "CINCO", "RISCO", "LOBO", "DADO", "DIFICIL", "MEDIO", "BAIXO", "FACIL", "RICOS" }, palavra;
	...
	public Dificil() {
		...
		lblFundo = new JLabel(new ImageIcon(getClass().getResource("img/Train/fundo3.png")));
		...
		for (int i = 0; i < 3; i++) {
			carro[i] = new ImageIcon(getClass().getResource("img/Train/car" + (i + 1) + "_dificil.png"));
		}

		for (int i = 0; i < 3; i++) {
			tipo[i] = rand.nextInt(3);
		...
		t[0] = new Timer(5, new EvtMoverCarro1());
		t[1] = new Timer(5, new EvtMoverCarro2());
		t[2] = new Timer(5, new EvtMoverCarro3());
		t[3] = new Timer(1, new EvtPontos());
		t[4] = new Timer(1, new EvtPontosNegativo());
		...
		btnSair.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(JOptionPane.showConfirmDialog(null, " Deseja Realmente Voltar? ", "Train!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION){
					...
					System.gc();
					Dificil.this.dispose();					
					Aplicacao frm = new Aplicacao();
			        frm.setSize(1024,768);
					frm.setLocationRelativeTo(Dificil.this);
					frm.setVisible(true);
	...
	private class RndTempo implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (r == 0 && ver[0] == false) {
				...
				Dificil.this.invalidate();
				Dificil.this.validate();
				Dificil.this.repaint();
	...
	public void addCarro(String letra, int nivel){
		if (nivel == 1){
			lblLetra[nivel-1].setBounds(1350 - carro[1].getIconWidth(), 35, carro[1].getIconWidth(), carro[1].getIconHeight());
			lblLetra[nivel-1].setFont(new Font("Trebuchet MS", 0, 60));
			...
			lblCarro[nivel-1].setBounds(1100 - carro[1].getIconWidth(), 30, carro[1].getIconWidth(), carro[1].getIconHeight());
		...
		if (nivel == 2){
			lblLetra[nivel-1].setText(letra);
			lblLetra[nivel-1].setBounds(1350 - carro[1].getIconWidth(), 275, carro[1].getIconWidth(), carro[1].getIconHeight());
			lblLetra[nivel-1].setFont(new Font("Trebuchet MS", 0, 60));
			lblLetra[nivel-1].setForeground(Color.white);
			lblLetra[nivel-1].setVisible(false);
			this.remove(lblFundo);
			...
			lblCarro[nivel-1].setBounds(1100 - carro[1].getIconWidth(), 270, carro[1].getIconWidth(), carro[1].getIconHeight());
			lblCarro[nivel-1].setVisible(false);
			this.remove(lblFundo);
			this.add(lblCarro[nivel-1]);
			this.add(lblFundo);
		...
		if (nivel == 3){
			lblLetra[nivel-1].setText(letra);
			lblLetra[nivel-1].setBounds(1350 - carro[1].getIconWidth(), 510, carro[1].getIconWidth(), carro[1].getIconHeight());
			lblLetra[nivel-1].setFont(new Font("Trebuchet MS", 0, 60));
			lblLetra[nivel-1].setForeground(Color.white);
			lblLetra[nivel-1].setVisible(false);
			this.remove(lblFundo);
			...
			lblCarro[nivel-1].setBounds(1100 - carro[1].getIconWidth(), 500, carro[1].getIconWidth(), carro[1].getIconHeight());
			lblCarro[nivel-1].setVisible(false);
			this.remove(lblFundo);
			this.add(lblCarro[nivel-1]);
			this.add(lblFundo);
			...
			if (String.valueOf(tecla).equalsIgnoreCase(lblLetra[i].getText().substring(0, 1))){
				...
				if (tipo[i] == 1) {
					lblCarro[i].setIcon(new ImageIcon(
							...
							"img/Train/car2_dificil_focus.png")));
				} else if (tipo[i] == 0){
					lblCarro[i].setIcon(new ImageIcon(
							getClass().getResource(
							"img/Train/car1_dificil_focus.png")));
				} else {
					lblCarro[i].setIcon(new ImageIcon(
							getClass().getResource(
					"img/Train/car3_dificil_focus.png")));
				}
				isFirstTime = false;
				break;
			}
				...
				if(String.valueOf(tecla).equalsIgnoreCase(lblLetra[i].getText().substring(0,1))){
					if(!(lblLetra[i].getText().length() <= 1)){
						...
						if (tipo[i] == 1){
							...
							lblLetra[i].setFont(new Font("Trebuchet MS", 0, 60));
